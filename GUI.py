"""
This file constructs and runs the final GUI
"""
...
import random
import threading
from algorithm.simulate import *
...
ASSETS_PATH = OUTPUT_PATH / Path("DSS/pics")
...
def change_map(choices, n, canvas, tracking_objects, images):
    """
    Function for "track orders" page
    Clicking an order changes which map is currently on screen
    """
...
def create_tracking_page(canvas, images, stats):
    ...
    """ Create 3 order button options """
    for j in range(3):
        ord = orders[len(orders)-j-1]
        ord_text = "Order                             #" + str(
...
def minus(vehicles, veh_num, i, j):
    """
    Remove vehicle from sector j for shift i and update label
    """
    if vehicles[j][i - 2] > 2:
...
def first_settings(j, clocks, cars, settings1):
    """
    Daily shift optimization setting
    """

    # if current setting is vehicle, set setting to time
...
def leave_page(canvas, objects):
    """
    Remove current page objects. A page needs to be cleared before we can create a new one
    """
